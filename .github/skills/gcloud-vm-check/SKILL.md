...
6. Run a safe harness validation path first:
   - `cargo check -p harness`
   - `cargo run -p harness -- analyze-suite --config configs/harness/demo_vod_live.json`
7. If the goal is full VPS validation, run a suite through `scripts/experiments/run_linux_vps_suite.sh`.
8. Inspect `results/raw/harness/` and `results/processed/harness/` for expected outputs.
9. Inspect ownership and permissions under `results/` after the run.
10. Report findings as: access, prerequisites, execution result, artifact result, permission result, and next actions.
