...
- Reconcile the canonical VPS matrix path with the current host-run fairness path so the repo documents one evidence workflow instead of a partly split one.
...
- Extend the legacy VPS docker runner so coexistence runs can share the same host-veth path as the fixed-preset matrix, or mark the host-run fairness path as permanently canonical.
