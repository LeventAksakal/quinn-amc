...
- raw transfer report under `results/local/raw/harness/*_report.json` or `results/vps/raw/harness/*_report.json`
- processed per-run AMC analysis under `results/local/processed/harness/*_amc.json` or `results/vps/processed/harness/*_amc.json`
- processed suite summary under `results/local/processed/harness/*_summary.json` or `results/vps/processed/harness/*_summary.json`
- processed suite comparison export under `results/local/processed/harness/*_comparison.json` or `results/vps/processed/harness/*_comparison.json`
...
```json
{
  "summary": {
    ...
    "report_path": "C:\\Code\\quinn-amc\\results\\local\\raw\\harness\\live_immediate_cubic_report.json"
  },
  "observations": [
    {
      "asset_name": "sintel_trailer",
...
```json
{
  ...
  "input_provenance": {
    ...
    "raw_report": {
      "path": "results/local/raw/harness/live_immediate_cubic_report.json",
      "sha256": "..."
    }
  },
  "aggregate": {
  ...
  "runs": [
    {
      ...
      "report_path": "results/local/raw/harness/live_immediate_cubic_report.json",
      "network_scenario": {
        "name": "local_loopback",
        "kind": "local"
      },
      "amc_analysis_path": "results/local/processed/harness/live_immediate_cubic_amc.json",
      "amc_aggregate": {
        "units_scored": 43,
        "average_media_utility_score": 0.009
      },
...
- Windows can execute `local` scenarios under `results/local/`, but `linux_tc_netem` scenarios only become runnable when the harness binary itself runs on Linux and writes or syncs under `results/vps/`.