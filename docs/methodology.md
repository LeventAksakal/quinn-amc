...
Phase 8 binds the final ratatui demo to one frozen raw report instead of treating the entire raw-results tree as a valid showcase pool.

- the canonical showcase raw report is `results/raw/harness/live_realtime_amc_preview_lte_constrained_report.json`
- the validated launch command is `cargo run -p harness -- live-demo --report results/raw/harness/live_realtime_amc_preview_lte_constrained_report.json --speed 1.0`
...
- the canonical Phase 8 raw showcase report under `results/raw/harness/live_realtime_amc_preview_lte_constrained_report.json` once copied from or regenerated by the frozen VPS matrix workflow
